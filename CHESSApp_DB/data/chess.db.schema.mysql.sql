...
CREATE TABLE IF NOT EXISTS `CHESS_DB`.`TypeMap` (
  `og_type` VARCHAR(45) NOT NULL,
  `std_type` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`og_type`),
  UNIQUE INDEX `rawType_UNIQUE` (`og_type` ASC) VISIBLE,
  UNIQUE INDEX `type_UNIQUE` (`std_type` ASC) VISIBLE)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `CHESS_DB`.`Gene` (
  ...
  CONSTRAINT `type`
    FOREIGN KEY (`type`)
    REFERENCES `CHESS_DB`.`TypeMap` (`std_type`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `CHESS_DB`.`TxDBXREF` (
  ...
  INDEX `fk_TxDBXREF_type_idx` (`type` ASC) VISIBLE,
