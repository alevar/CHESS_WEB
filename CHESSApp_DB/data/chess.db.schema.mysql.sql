...
USE `CHESS_DB`$$
DROP TRIGGER IF EXISTS `CHESS_DB`.`TxDBXREF_AFTER_INSERT_updateLocus` $$
USE `CHESS_DB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `CHESS_DB`.`TxDBXREF_AFTER_INSERT_updateLocus` AFTER INSERT ON `TxDBXREF` FOR EACH ROW
BEGIN
    DECLARE locus_start INT UNSIGNED;
    DECLARE locus_end INT UNSIGNED;

    -- Retrieve start and end values from the TxDBXREF entry
    SELECT NEW.start, NEW.end INTO locus_start, locus_end;

    -- Update the corresponding Locus entry
    UPDATE `CHESS_DB`.`Locus`
    SET start = locus_start, end = locus_end
    WHERE lid = (
        SELECT lid
        FROM `CHESS_DB`.`Gene`
        WHERE gid = NEW.gid
