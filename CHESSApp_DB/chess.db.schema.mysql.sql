...
CREATE TABLE IF NOT EXISTS `CHESS_DB`.`tx_attribute` (
  ...
  `value_cat` VARCHAR(45) CHARACTER SET 'ascii' COLLATE 'ascii_bin' NOT NULL DEFAULT '',
...
USE `CHESS_DB`$$
DROP TRIGGER IF EXISTS `CHESS_DB`.`enforce_single_active_insert_AFTER_INSERT` $$
USE `CHESS_DB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `CHESS_DB`.`enforce_single_active_insert_AFTER_INSERT` AFTER INSERT ON `configurations` FOR EACH ROW
BEGIN
...
USE `CHESS_DB`$$
DROP TRIGGER IF EXISTS `CHESS_DB`.`enforce_single_active_update_AFTER_UPDATE` $$
USE `CHESS_DB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `CHESS_DB`.`enforce_single_active_update_AFTER_UPDATE` 
AFTER UPDATE ON `configurations` FOR EACH ROW
BEGIN
