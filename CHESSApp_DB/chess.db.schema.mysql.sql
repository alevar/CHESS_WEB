...
CREATE TABLE IF NOT EXISTS `CHESS_DB`.`TxDBXREF` (
  ...
  `type` ENUM("protein_coding", "non_coding", "antisense", "pseudo", "trna", "mirna", "snorna", "snrna", "rrna", "sirna") NULL,
  `cds` TEXT NULL,
...
CREATE TABLE IF NOT EXISTS `CHESS_DB`.`SequenceIDMap` (
  `assemblyName` VARCHAR(45) NOT NULL,
  `sequenceID` VARCHAR(45) NOT NULL COMMENT 'sequenceID on the main assembly',
  `alternativeID` VARCHAR(45) NOT NULL COMMENT 'another name for the name',
  `nomenclature` VARCHAR(45) NOT NULL COMMENT 'name of the nomenclature',
  PRIMARY KEY (`assemblyName`, `sequenceID`, `alternativeID`, `nomenclature`),
  CONSTRAINT `fk_SequenceIDMap_assembly`
    FOREIGN KEY (`assemblyName` , `sequenceID`)
    REFERENCES `CHESS_DB`.`SequenceIDs` (`assemblyName` , `sequenceID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
